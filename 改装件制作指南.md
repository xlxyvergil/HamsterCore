...
| quality | float | 品质系数，影响生成的属性值 |
| dimensions | Set<ResourceLocation> | 适用维度，空列表表示所有维度 |
| category | String | 类别，目前仅支持"weapon" |
...
| applicableCategories | List<WeaponCategory> | 适用武器分类，空列表表示所有分类 |
| applicableTypes | List<WeaponType> | 适用武器类型，空列表表示所有类型 |
| mutualExclusionGroups | Set<String> | 互斥组，同一组的改装件只能装一个 |
...
| 字段名 | 类型 | 说明 |
|-------|------|------|
| type | ResourceLocation | 词缀类型，命名空间+属性名，如"hamstercore:cold_damage" |
| source | String | 属性名，如"cold_damage" |
| value | float | 属性值 |
...
```json
{
  ...
  "affixes": [
    ...
    {
      "type": "attributeslib:crit_chance",
      "source": "crit_chance",
      "value": 0.05,
      "operation": "ADDITION"
    }
